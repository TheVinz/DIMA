...
class _NestedScrollViewState extends State<CustomNestedScrollView> {
...
class _NestedScrollCoordinator implements ScrollActivityDelegate, ScrollHoldController {
  ...
  void goBallistic(double velocity) {
    if(!_outerPosition.physics.shouldAcceptUserOffset(_outerPosition)) {
      beginActivity(
        createOuterBallisticScrollActivity(0),
            (_NestedScrollPosition position) => createInnerBallisticScrollActivity(position, 0),
      );
      return;
    }
    beginActivity(
  ...
  void applyUserOffset(double delta) {
    if(!_outerPosition.physics.shouldAcceptUserOffset(_outerPosition)) return;
